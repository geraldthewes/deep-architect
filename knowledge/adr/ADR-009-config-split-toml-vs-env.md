...
**`~/.deep-researcher.toml`** controls behavior:
...
- **Secrets never in files:** Storing API keys in `~/.deep-researcher.toml` is a security risk (checked into git accidentally, readable by other processes). Environment variables are the standard (12-factor app principle).
...
- `load_config()` raises `FileNotFoundError` with a clear message if `~/.deep-researcher.toml` is missing.
...
- The `.deep-researcher.toml.template` documents all TOML keys but never includes auth/endpoint fields.

**Files:** `deep_researcher/config.py`, `deep_researcher/agents/client.py:68-94`, `.deep-researcher.toml.template`
