...
├── idlc.yaml           # IDLC workflow config (ticket stages, artifact gates)
...
**Key relationships**: Ticket IDs (e.g., `PROJ-0009`) link artifacts across directories — a ticket in `tickets/` may have linked research in `research/` and a plan in `plans/`.
